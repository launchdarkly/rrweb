...
function requireBuiltRecordBundle() {
  if (!existsSync(recordJsPath)) {
    throw new Error(
      'Missing packages/record/dist/rrweb-record.js. Run `yarn workspace @highlight-run/rrweb-record build` before running this test.',
    );
  }
}
...
function emittedJavaScriptFiles() {
  if (!existsSync(distDir)) {
    throw new Error(
      'Missing packages/record/dist. Run `yarn workspace @highlight-run/rrweb-record build` before running this test.',
    );
  }
...
describe('record', () => {
  ...
  // The fork inlines its workspace dependencies (rrweb-snapshot, rrdom, ...)
  // into every bundle via vite.config.default.ts aliases, so upstream's
  // postcss tree-shaking and bundle-size guarantees do not apply here.
  it.skip('does not bundle replay-only postcss code', () => {
  ...
  it.skip('keeps the ESM record bundle at least 200 KiB below the baseline size', () => {
    requireBuiltRecordBundle();
